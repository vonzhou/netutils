...
int main(int argc, char **argv){
	...
	to = (struct sockaddr_in *)&whereto;  // server socket addr
	to->sin_family = AF_INET;

	/* try to convert as dotted decimal address,
	   else if that fails assume it's a hostname 
	把点分十进制的IP地址转为网络序的二进制形式,
	利用该函数的返回值来判断给出的target是否是主机名*/
	to->sin_addr.s_addr = inet_addr(target);// FIXME
	if (to->sin_addr.s_addr != (u_int)-1)
		hostname = target;
	else {
		hp = gethostbyname(target);//deprecated!!
		if (!hp) {
...
/*
 * Send the request from the socket fd s.
 */
void sender(int s){
	int		i, cc;
	struct icmp	*icp;
 ...
 */
int procpack(char *buf, int cc, struct sockaddr_in from){
	int		i, hlen;
	struct icmp	*icp;
	struct ip	*ip;
	...
	if (icp->icmp_type == ICMP_TSTAMPREPLY) {
		...
		// 20 = icmp header(8) + data(12, 3 timestamp fields)
		if (cc != 20) 
